import pandas as pd
import numpy as np
from src.features.technical_indicators import add_technical_features, calculate_log_returns, calculate_volatility
...
def test_add_technical_features_structure():
    ...
    pd.testing.assert_series_equal(df_features['volatility_21d'], manual_vol, obj="Volatility Series", check_names=False)

    # Verify log returns match manual calculation
    manual_log_ret = calculate_log_returns(df['Close'])
    pd.testing.assert_series_equal(df_features['log_returns'], manual_log_ret, obj="Log Returns Series", check_names=False)
